...
a, b, c, d = int(input()), int(input()), int(input()), int(input())  # получаем числовые значения на ввод
if a <= b <= 10 and c <= d <= 10: # проверка на соответствие условию
    for hdr in range(c, d + 1):  # цикл для формирования заголовка таблицы
        print(' ', hdr, sep='\t', end='')
    print(end='\n')  # вывод новой строки
    for i in range(a, b + 1):  # цикл для формирования строк
        print(i, '\t', end='')
        for j in range(c, d + 1):  # цикл для формирования значений в столбцах
            print(i * j, end='\t')
        print(end='\n')
else:
    print('Не выполнено условие: значения больше 10')
